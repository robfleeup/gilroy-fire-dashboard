...
def fetch_json(url: str, timeout: int = 12):
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "GilroyFireOperations/3.0",
    ...
    unit_ids = sorted(
        {
            ...
            if re.match(r"^(E|T|RM|B)\d+", u.upper())
        }
    )
    ...
    return jsonify(
        {
            ...
            "chiefs_committed": sum(1 for u in unit_ids if u.startswith("B")),
            "als_assigned": sum(1 for u in ambulance_ids if u.startswith("M")),
            "bls_assigned": sum(1 for u in ambulance_ids if u.startswith("A")),
...
def weather():
    try:
        ...
        return jsonify(
            {
                "available": True,
                "temperature_f": hourly.get("temperature"),
                "description": hourly.get("shortForecast"),
                "wind_mph": hourly.get("windSpeed"),
                "wind_direction": hourly.get("windDirection"),
                "humidity": (hourly.get("relativeHumidity") or {}).get("value"),
                "probability": (
                    hourly.get("probabilityOfPrecipitation") or {}
                ).get("value"),
                "observed_at": hourly.get("startTime"),
            }
        )
    except Exception as exc:
